...
Beyond Controls is a local replacement for the packwiz-managed upstream Controlify jar. Its internal mod ID deliberately remains `controlify` for API, resource-pack, config, and addon compatibility. The sync tool builds its 1.21.1 NeoForge target in a temporary Stonecutter source copy, overwrites the packwiz-managed Controlify filename, and removes the legacy `controlify-local.jar` destination only after the replacement copy succeeds. If local syncing is skipped, packwiz continues to provide upstream Controlify as a fallback. Its current Gradle plugins require the machine-local `java25Home` setting for this isolated build; Minecraft and the rest of the pack continue to use the configured Java 21 runtime.
